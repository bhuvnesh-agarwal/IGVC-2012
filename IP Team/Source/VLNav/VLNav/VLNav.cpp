...
bool isNear(location a, location b)
{
	...
	if((abs(a.x - b.x) <= 10) && (a.y >= 0.4*b.y) && (abs(a.theta - b.theta) < 10))
		return true;
	else
		return false;
}
...
void main(int argc, char** argv)
{
	...
	//dstQuad[0].x = 217; //src Top left
	//dstQuad[0].y = 67;
	//dstQuad[1].x = 400; //src Top right
	//dstQuad[1].y = 67;
	//dstQuad[2].x = 10; //src Bottom left
	//dstQuad[2].y = 171;
	//dstQuad[3].x = 600; //src Bot right
	//dstQuad[3].y = 171;
	...
	while(1)
	{
		...
		IplImage* img = cvLoadImage( "../../Data/4 Feb/path not being formed sometimes.jpg", CV_LOAD_IMAGE_COLOR);
		cvShowImage( "Source", img );
		//cvWriteFrame(writer, img);
		//cvSaveImage(nameGen(ik++), img, 0);
		//cvSaveImage("path not formed by probability.jpg", img, 0);
		...
		CvSeq* results =  cvHoughLines2(warp_img, line_storage, CV_HOUGH_PROBABILISTIC, 10, CV_PI/180*5, 150, 70, 10);
		...
		/*
		fp = fopen("lwHist.txt", "w");
		fprintf(fp, "%d", lwSum/nopf);
		fclose(fp);*/
	}
}
